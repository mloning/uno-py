...
def _generate_test_players(n_players: int = 3) -> list[Player]:
    return [Player(name=str(i)) for i in range(1, n_players + 1)]
...
def test_players_skip() -> None:
    ...
    assert players.next().name == "3"
    assert players.next().name == "1"
    players.skip()  # skipping player B
    assert players.next().name == "3"


def test_players_double_reverse_idempotent() -> None:
...
def test_players_reverse_4_players() -> None:
    players = _generate_test_players(n_players=4)
    players = Players(players=players)
...
def test_players_reverse_5_players() -> None:
    players = _generate_test_players(n_players=5)
    players = Players(players=players)
